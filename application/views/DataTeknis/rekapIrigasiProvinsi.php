				...
				<div class="card">
					<div class="card-body">
						<div class="text-center">
							<h3 class="font-weight-bolder">REKAPITULASI DOKUMEN DATA TEKNIS IRIGASI PROVINSI TA. <?= $this->session->userdata('thang'); ?></h3>
						</div>
						<br>
						<div>
						...
						<table class="table table-bordered mt-4">
							...
							<tbody id="tbody_data">
								...
								<?php $no=1; foreach ($dataRekap as $key => $val) { ?>
									<tr>
										...
										<td><a href="<?= base_url(); ?>DataTeknis/rekapIrigasiKabKota/<?= $val->provid ?>"><?= $val->provinsi ?></a></td>
										<td class="text-right"><?= $val->dataTeknis; ?></td>
										<td class="text-right"><?= $val->rk; ?></td>
							...
							<tfoot>
								<tr style="background-color:#d9dee2;">
									...
									<th class="text-right"><b><?= $dataTeknis; ?></b></th>
									<th class="text-right"><b><?= $rk; ?></b></th>
...
<script type="text/javascript">
	$( document ).ready(function() {
		$('#selectRekap').on('change', function() {
			...
			if (val == 2) {
				window.location.href = base_url()+"DataTeknis/rekapPengendaliBanjirProvinsi";
			}
