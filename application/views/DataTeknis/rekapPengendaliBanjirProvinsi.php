				...
				<div class="card">
					<div class="card-body">
						<div class="text-center">
							<h3 class="font-weight-bolder">REKAPITULASI DOKUMEN DATA TEKNIS PENGENDALI BANJIR PROVINSI TA. <?= $this->session->userdata('thang'); ?></h3>
						</div>
						<br>
						<div>
							...
							<tbody id="tbody_data">
								...
								<?php $no=1; foreach ($dataRekap as $key => $val) { ?>
									<tr>
										...
										<td><a href="<?= base_url(); ?>DataTeknis/rekapPengendaliBanjirKabKota/<?= $val->provid ?>"><?= $val->provinsi ?></a></td>
										<td class="text-right"><?= $val->rk_pb; ?></td>
									...
									$dataTeknis += (int)$val->rk_pb;
									$rk += (int)$val->lembar_ck_pb;
									$sid += (int)$val->sid_pb;
									$ded += (int)$val->ded_pb;
									$kak += (int)$val->kak_pb;
...
<script type="text/javascript">
	$( document ).ready(function() {
		$('#selectRekap').on('change', function() {
			...
			if (val == 1) {
				window.location.href = base_url()+"DataTeknis/rekapIrigasiProvinsi";
			}
