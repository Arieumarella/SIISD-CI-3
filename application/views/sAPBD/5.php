			...
			<div class="row m-0 p-0 text-left">
				...
				<div class="col-sm-12 mt-2">
					...
					<div class="row">
						...
						<div class="row col-sm-12  col-lg-4 p-0">
							...
							<?php if ($this->session->userdata('prive') == 'admin') { ?>
								<a href="<?= base_url(); ?>FormTeknis1F/formExcel" class="btn btn-info ml-0" aksi="add" title="Unduh"><i class="fas fa-file-excel"></i> Unduh</a>
							<?php } ?>
							...
							<?php if ($this->session->userdata('prive') == 'admin' or $this->session->userdata('prive') == 'pemda' or $this->session->userdata('prive') == 'provinsi') { ?>
								...
								<a href="<?= base_url(); ?>FormTeknis1F/formExcel" class="btn btn-success mr-1" aksi="add" title="Download"><i class="fas fa-file-excel"></i> Format Excel</a>
					...
					<table class="table table-bordered">
						...
						<thead id="thead_data">
							<tr id="boxThField0" style="background-color:#18978F; color:#fff;">
								...
								<th style="border: thin solid #006666;" colspan="1" rowspan="1" class="text-center">Provinsi</th>
								<th style="border: thin solid #006666;" colspan="1" rowspan="1" class="text-center">Kota/Kabupaten</th>
								<th style="border: thin solid #006666;" colspan="1" rowspan="1" class="text-center">No. DPA</th>
								<th style="border: thin solid #006666;" colspan="1" rowspan="1" class="text-center">Tanggal</th>
							</tr>
							...
							<tr id="boxThNo" style="background-color:#18978F; color:#fff;">
								<th style="border: thin solid #006666;" class="text-center">1</th>
								<th style="border: thin solid #006666;" class="text-center">2</th>
								<th style="border: thin solid #006666;" class="text-center">3</th>
								<th style="border: thin solid #006666;" class="text-center">4</th>
								<th style="border: thin solid #006666;" class="text-center">5</th>
							</tr>
						</thead>
	...
	$(document).ready(function() {
		...
		getDataTabel = async function(page = null, providX = null, kotakabidX = null) {
			try {
				...
				ajaxUntukSemua(base_url() + 'SharingAPBD/getDataTable', {
					perhalaman,
					halamanSaatIni,
					search,
					provid,
		...
		function setTabelKonten(data) {
			...
			$.each(data, function(key, value) {
				...
				<td id="irigasiid_50581" style="border: thin solid #006666;" class="options menuALink"><a href="${base_url()}SharingAPBD/getDetailData/${value.id}">${value.noDpa}</a></td>
				<td id="laBaku_50581" style="border: thin solid #006666;" class="">${value.tanggal}</td>
				</tr>`;
		...
		$('#prov').change(function() {
			...
			ajaxUntukSemua(base_url() + 'SharingAPBD/getDataKabKota', {
				prov
			}, function(data) {
