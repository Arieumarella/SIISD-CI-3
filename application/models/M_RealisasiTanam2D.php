...
class M_RealisasiTanam2D extends CI_Model {
	...
	public function getDataTable($jumlahDataPerHalaman, $search, $offset, $provid, $kotakabid)
	{
		...
		$qry = "SELECT d.provinsi, a.irigasiid as irigasiidX, c.kemendagri, a.nama, b.* FROM (SELECT * FROM m_irigasi WHERE isActive = '1') AS a
	...
	public function getDataDiById($id='')
	{
		...
		$qry = "SELECT a.nama, a.irigasiid as irigasiidX, b.* FROM (SELECT * FROM m_irigasi WHERE isActive = '1') AS a LEFT JOIN (SELECT * FROM p_f2d WHERE ta='$thang') AS b on a.irigasiid=b.irigasiid WHERE a.irigasiid='$id'";
	...
	public function getDataDownload($ta, $privem, $kotakabidx=null)
	{
		...
		if ($kotakabidx == null) {
			if ($prive == 'admin') {
				...
				$qry = "SELECT d.provinsi, c.kemendagri, b.nama, a.* FROM p_f2d AS a
