...
class M_DataTeknis extends CI_Model {
	...
	public function DataTeknisForm($kotakabid, $thang)
	{
		...
		LEFT JOIN (SELECT kotakabid, path AS dataTeknis FROM m_data_teknis WHERE kotakabid='$kotakabid' AND ta='$thang' AND jns_file='dataTeknis') AS b ON a.kotakabid=b.kotakabid
		LEFT JOIN (SELECT kotakabid, path AS rk FROM m_data_teknis WHERE kotakabid='$kotakabid' AND ta='$thang' AND jns_file='rk') AS c ON a.kotakabid=c.kotakabid
	...
	{
		$qry = "SELECT * FROM (SELECT * FROM m_kotakab WHERE kotakabid='$kotakabid') AS a
		LEFT JOIN (SELECT kotakabid, path AS rk_pb FROM m_data_teknis WHERE kotakabid='$kotakabid' AND ta='$thang' AND jns_file='rk_pb') AS b ON a.kotakabid=b.kotakabid
	...
	public function rekapIrigasiProvinsi()
	{
		...
		$qry = "SELECT a.*, dataTeknis, rk, sid, ded, kak, skema_jaringan, skema_bangunan, bc_volume, rab, smk3, dpa, dokumentasi, kebenaran_data, pemenuhan_kriteria, penyiapan_lahan, kesanggupan_op FROM (SELECT * FROM m_prov) AS a
		LEFT JOIN (SELECT provid, COUNT(*) AS dataTeknis FROM m_data_teknis WHERE ta='$ta' AND jns_file='dataTeknis' GROUP BY provid) AS b ON a.provid=b.provid
		LEFT JOIN (SELECT provid, COUNT(*) AS rk FROM m_data_teknis WHERE ta='$ta' AND jns_file='rk' GROUP BY provid) AS c ON a.provid=c.provid
	...
	public function rekapIrigasiKabKota($idprov)
	{
		...
		LEFT JOIN (SELECT kotakabid, id AS id_dataTeknis, path AS path_dataTeknis, ekstensi AS ekstensi_dataTeknis, created_at as upload_time_dataTeknis FROM m_data_teknis WHERE provid='$idprov' AND ta='$thang' AND jns_file='dataTeknis') AS b ON a.kotakabid=b.kotakabid
		LEFT JOIN (SELECT kotakabid, id AS id_rk, path AS path_rk, ekstensi AS ekstensi_rk, created_at as upload_time_rk  FROM m_data_teknis WHERE provid='$idprov' AND ta='$thang' AND jns_file='rk' ) AS c ON a.kotakabid=c.kotakabid
	...
	public function rekapPengendaliBanjirProvinsi()
	{
		...
		LEFT JOIN (SELECT provid, COUNT(*) AS rk_pb FROM m_data_teknis WHERE ta='$ta' AND jns_file='rk_pb' GROUP BY provid) AS b ON a.provid=b.provid
	...
	public function rekapPengendaliBanjirKabKota($idprov)
	{
		...
		$qry = "SELECT * FROM (SELECT kotakabid, provid, kemendagri FROM m_kotakab WHERE provid='$idprov') AS a
		LEFT JOIN (SELECT kotakabid, id AS id_rk_pb, path AS path_rk_pb, ekstensi AS ekstensi_rk_pb, created_at as upload_time_rk_pb FROM m_data_teknis WHERE provid='$idprov' AND ta='$thang' AND jns_file='rk_pb') AS b ON a.kotakabid=b.kotakabid
