...
class M_IndexKinerja4A extends CI_Model {
	...
	public function getDataTable($jumlahDataPerHalaman, $search, $offset, $provid, $kotakabid)
	{
		...
		LEFT JOIN (SELECT * FROM p_f4a WHERE ta='$ta') AS b ON a.irigasiid=b.irigasiid
		LEFT JOIN m_prov as d on a.provid=d.provid
		LEFT JOIN m_kotakab as c on a.kotakabid=c.kotakabid
		LEFT JOIN (SELECT irigasiid, buBendung as embung1a FROM p_f1a WHERE ta=$ta) as e on a.irigasiid=e.irigasiid
	...
	public function getDataDiById($id='')
	{
		...
		$qry = "SELECT a.nama, a.irigasiid as irigasiidX, b.*, a.lper FROM  (SELECT * FROM m_irigasi WHERE isActive = '1') AS a LEFT JOIN (SELECT * FROM p_f4a WHERE ta='$thang') AS b on a.irigasiid=b.irigasiid WHERE a.irigasiid='$id'";
	...
	public function getDataDiFull($thangX, $kab)
	{
		...
		$qry = "SELECT b.provinsi, c.kemendagri, a.nama, d.*, a.provid as provIdX, a.irigasiid as irigasiidX,  a.kotakabid as kotakabidX, a.lper FROM (
			SELECT * FROM m_irigasi WHERE isActive = '1' AND kotakabid='$kab' AND kategori='DI') AS a LEFT JOIN m_prov as b on a.provid=b.provid LEFT JOIN m_kotakab as c on a.kotakabid=c.kotakabid LEFT JOIN (SELECT * FROM p_f4a WHERE ta='$thangX' AND kotakabid='$kab') as d on a.irigasiid=d.irigasiid ";
	...
	public function getDataDownload($ta, $prive, $kotakabidx=null)
	{
		...
		if ($kotakabidx==null) {
			...
			if ($prive == 'admin') {
				...
				$qry = "SELECT d.provinsi, c.kemendagri, b.nama, a.* FROM p_f4a AS a
