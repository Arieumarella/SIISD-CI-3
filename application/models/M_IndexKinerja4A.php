...
class M_IndexKinerja4A extends CI_Model {
	...
	public function getDataTable($jumlahDataPerHalaman, $search, $offset, $provid, $kotakabid)
	{
		...
		$qry = "SELECT d.provinsi, c.kemendagri, a.irigasiid as irigasiidX, a.nama, b.* FROM (SELECT * FROM m_irigasi WHERE isActive = '1') AS a
	...
	public function getDataDiById($id='')
	{
		...
		$qry = "SELECT a.nama, a.irigasiid as irigasiidX, b.* FROM  (SELECT * FROM m_irigasi WHERE isActive = '1') AS a LEFT JOIN (SELECT * FROM p_f4a WHERE ta='$thang') AS b on a.irigasiid=b.irigasiid WHERE a.irigasiid='$id'";
