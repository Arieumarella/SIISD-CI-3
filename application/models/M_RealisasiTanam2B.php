...
class M_RealisasiTanam2B extends CI_Model {
	...
	public function getDataTable($jumlahDataPerHalaman, $search, $offset, $provid, $kotakabid)
	{
		...
		$qry = "SELECT d.provinsi, c.kemendagri, b.nama, b.irigasiid as irigasiidX, a.* FROM (SELECT * FROM m_irigasi WHERE isActive = '1' $cari LIMIT $jumlahDataPerHalaman OFFSET $offset) AS b
		LEFT JOIN (SELECT * FROM p_f2b WHERE ta=$ta) AS a ON a.irigasiid=b.irigasiid
	...
	public function getDataDiById($id='')
	{
		...
		$qry = "SELECT a.nama, a.irigasiid as irigasiidX, b.*, a.lper FROM (SELECT * FROM m_irigasi WHERE isActive = '1') AS a LEFT JOIN (SELECT * FROM p_f2b WHERE ta='$thang') AS b on a.irigasiid=b.irigasiid WHERE a.irigasiid='$id'";
	...
	public function getDataDownload($ta, $prive, $kotakabidx=null)
	{
		...
		if ($kotakabidx == null) {
			...
			if ($prive == 'admin') {
				...
				$qry = "SELECT d.provinsi, c.kemendagri, b.nama, b.irigasiid as irigasiidX, b.lper, a.* FROM (SELECT * FROM m_irigasi WHERE isActive = '1' $cari ) AS b
				LEFT JOIN (SELECT * FROM p_f2b WHERE ta=$ta) AS a ON a.irigasiid=b.irigasiid
				LEFT JOIN m_prov as d on b.provid=d.provid
				LEFT JOIN m_kotakab as c on b.kotakabid=c.kotakabid
				ORDER BY d.provinsi, c.kemendagri";
