...
class M_Form9 extends CI_Model {
	...
	public function getDataTable($jumlahDataPerHalaman, $search, $offset, $provid, $kotakabid)
	{
		...
		$cariEpaksi = ($kotakabid != null) ? " WHERE k_kabupaten='$kotakabid'" : '';
		...
		LEFT JOIN 
		(select kode_di,m.* FROM (SELECT k_di,IF(tipe_key='I',bobot,0) as I,IF(tipe_key='nf_2',bobot,0) as nf_2,IF(tipe_key='nf_3',bobot,0) as nf_3,IF(tipe_key='nf_4',bobot,0) as nf_4,IF(tipe_key='nf_5',bobot,0) as nf_5,IF(tipe_key='nf_6',bobot,0) as nf_6 FROM `epaksi_f9` $cariEpaksi) as m 
			LEFT JOIN 
	...
	public function getDataDiById($id='')
	{
		...
		$qry = "SELECT a.nama, a.irigasiid as irigasiidX, b.* FROM (SELECT * FROM m_irigasi WHERE isActive = '1') AS a LEFT JOIN (SELECT * FROM p_f9 WHERE ta='$thang') AS b on a.irigasiid=b.irigasiid WHERE a.irigasiid='$id'";
