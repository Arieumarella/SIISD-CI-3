...
class M_RealisasiTanam2C extends CI_Model {
	...
	public function getDataTable($jumlahDataPerHalaman, $search, $offset, $provid, $kotakabid)
	{
		...
		$qry = "SELECT d.provinsi, b.irigasiid as irigasiidX, c.kemendagri, nama, a.* FROM (SELECT * FROM m_irigasi WHERE isActive = '1') AS b
	...
	public function getDataDiById($id='')
	{
		...
		$qry = "SELECT a.nama, a.irigasiid as irigasiidX, b.* FROM (SELECT * FROM m_irigasi WHERE isActive = '1') AS a LEFT JOIN (SELECT * FROM p_f2c WHERE ta='$thang') AS b on a.irigasiid=b.irigasiid WHERE a.irigasiid='$id'";
