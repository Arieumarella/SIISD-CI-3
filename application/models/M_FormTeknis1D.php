...
class M_formTeknis1D extends CI_Model {
	...
	public function getDataTable($jumlahDataPerHalaman, $search, $offset, $provid, $kotakabid)
	{
		...
		LEFT JOIN (SELECT * FROM p_f1d WHERE ta=$ta order by irigasiid)  AS a ON a.irigasiid=b.irigasiid
	...
	public function getDataDownload($ta, $prive, $kotakabidx=null)
	{
		...
		if ($kotakabidx == null) {
			...
			if ($prive == 'admin') {
				...
				FROM (SELECT * FROM m_irigasi WHERE isActive = '1' AND kategori='DIT') AS b
				...
				FROM (SELECT * FROM m_irigasi WHERE isActive = '1' AND kategori='DIT' AND kotakabid='$kotakabid') AS b
				LEFT JOIN (SELECT * FROM p_f1d WHERE ta=$ta AND kotakabid='$kotakabid')  AS a ON a.irigasiid=b.irigasiid
				LEFT JOIN m_prov as d on b.provid=d.provid
				LEFT JOIN m_kotakab as c on b.kotakabid=c.kotakabid
				LEFT JOIN 
				(SELECT kode_di,m.* 
			...
			FROM (SELECT * FROM m_irigasi WHERE isActive = '1' AND kategori='DIT' AND kotakabid='$kotakabidx') AS b
			LEFT JOIN (SELECT * FROM p_f1d WHERE ta=$ta AND kotakabid='$kotakabidx')  AS a ON a.irigasiid=b.irigasiid
			LEFT JOIN m_prov as d on b.provid=d.provid
			LEFT JOIN m_kotakab as c on b.kotakabid=c.kotakabid
			LEFT JOIN 
			(SELECT kode_di,m.* 
