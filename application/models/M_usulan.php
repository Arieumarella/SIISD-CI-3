...
class M_usulan extends CI_Model
{
	...
	public function rekapCehklistSimoni()
	{
		...
		LEFT JOIN (SELECT kdprov, COUNT(*) AS jml_data, SUM(IF(verif_balai=1,1,0)) AS jml_balai, SUM(IF(verif_sda=1,1,0)) AS jml_sda, SUM(IF(verif_pusat=1,1,0)) AS jml_pusat FROM m_usulan_simoni WHERE ta=$ta GROUP BY kdprov) AS b ON a.provid=b.kdprov";
	...
	public function rekapCehklistSimoniKabKota($idProv)
	{
		...
		LEFT JOIN (SELECT kdkabkota, COUNT(*) AS jml_data, SUM(IF(verif_balai=1,1,0)) AS jml_balai, SUM(IF(verif_sda=1,1,0)) AS jml_sda, SUM(IF(verif_pusat=1,1,0)) AS jml_pusat FROM m_usulan_simoni WHERE ta=$ta GROUP BY kdkabkota) AS b ON a.kotakabid=b.kdkabkota";
