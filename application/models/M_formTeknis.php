...
class M_formTeknis extends CI_Model {
	...
	public function getDataTable($jumlahDataPerHalaman, $search, $offset, $provid, $kotakabid)
	{
		...
		FROM (SELECT * FROM m_irigasi WHERE isActive = '1' $cari order by irigasiid LIMIT $jumlahDataPerHalaman OFFSET $offset) AS b
		LEFT JOIN (SELECT * FROM p_f1a WHERE ta=$ta order by irigasiid) AS a ON a.irigasiid=b.irigasiid
		...
		(SELECT kode_di,m.* 
			FROM
			(SELECT k_di,
				SUM(IF(k_aset='B01',qty,0)) AS B01,
				SUM(IF(k_aset='B03',qty,0)) AS B03,
				SUM(IF(k_aset='B04',qty,0)) AS B04,
				SUM(IF(k_aset='S01',qty,0)) AS S01,
				SUM(IF(k_aset='S02',qty,0)) AS S02,SUM(IF(k_aset='S15',qty,0)) AS S15,SUM(IF(k_aset='S11',qty,0)) AS S11,SUM(IF(k_aset='P01',qty,0)) AS P01,SUM(IF(k_aset='P02',qty,0)) AS P02,SUM(IF(k_aset='P03',qty,0)) AS P03,SUM(IF(k_aset='C03',qty,0)) AS C03,SUM(IF(k_aset='C02',qty,0)) AS C02,SUM(IF(k_aset='C04',qty,0)) AS C04,SUM(IF(k_aset='C07',qty,0)) AS C07,SUM(IF(k_aset='C11',qty,0)) AS C11,SUM(IF(k_aset='C17',qty,0)) AS C17,SUM(IF(k_aset='C05',qty,0)) AS C05,SUM(IF(k_aset='C08',qty,0)) AS C08,SUM(IF(k_aset='C14',qty,0)) AS C14,SUM(IF(k_aset='S12',qty,0)) AS S12,SUM(IF(k_aset='C21',qty,0)) AS C21,SUM(IF(k_aset='C22',qty,0)) AS C22,SUM(IF(k_aset='S21',qty,0)) AS S21,SUM(IF(k_aset='C06',qty,0)) AS C06,SUM(IF(k_aset='F01',qty,0)) AS F01,SUM(IF(k_aset='F03',qty,0)) AS F03,SUM(IF(k_aset='F11',qty,0)) AS F11,SUM(IF(k_aset='C01',qty,0)) AS C01 
				FROM epaksi_f1  GROUP BY k_di) AS m 
			LEFT JOIN 
			(SELECT * FROM m_mapping_di GROUP BY k_di) AS n ON m.k_di=n.k_di) AS e ON b.irigasiid=e.kode_di
	...
	public function getDataDownload($ta, $prive, $kotakabidx=null)
	{
		if ($kotakabidx == null) {
			...
			if ($prive == 'admin') {
				...
				FROM (SELECT * FROM m_irigasi WHERE isActive = '1' AND kategori='DI') AS b
				...
				FROM (SELECT * FROM m_irigasi WHERE isActive = '1' AND kotakabid='$kotakabid' AND kategori='DI') AS b
				LEFT JOIN (SELECT * FROM p_f1a WHERE ta=$ta AND kotakabid='$kotakabid') AS a ON a.irigasiid=b.irigasiid
				LEFT JOIN m_prov as d on b.provid=d.provid
				LEFT JOIN m_kotakab as c on b.kotakabid=c.kotakabid
				LEFT JOIN 
				(SELECT kode_di,m.* 
			...
			FROM (SELECT * FROM m_irigasi WHERE isActive = '1' AND kotakabid='$kotakabidx' AND kategori='DI') AS b
			LEFT JOIN (SELECT * FROM p_f1a WHERE ta=$ta AND kotakabid='$kotakabidx') AS a ON a.irigasiid=b.irigasiid
			LEFT JOIN m_prov as d on b.provid=d.provid
			LEFT JOIN m_kotakab as c on b.kotakabid=c.kotakabid
			LEFT JOIN 
			(SELECT kode_di,m.* 
